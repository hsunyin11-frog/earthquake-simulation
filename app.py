...
if st.session_state.simulated:
    with st.spinner("正在計算受災程度與分配避難所..."):
        ...
        for _, row in shelter_df.iterrows():
            shelters.append(Shelter(id=str(row["id"]), name=str(row["name"]), lat=row["lat"], lon=row["lon"], capacity=row["capacity"], city=row.get("city", ""), district=row.get("district", ""), village=row.get("village", "")))
