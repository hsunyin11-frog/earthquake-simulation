...
# 改進：使用快速選擇按鈕而非文字輸入
st.sidebar.write("**地震規模快速選擇：**")
mag_cols = st.sidebar.columns(4)
mag_buttons = [4.0, 5.0, 6.0, 7.0]
selected_mag = None

for idx, (col, mag_val) in enumerate(zip(mag_cols, mag_buttons)):
    if col.button(f"{mag_val}", key=f"mag_{mag_val}"):
        selected_mag = mag_val
        st.session_state.selected_magnitude = mag_val

# 保持使用者上次選擇的規模，或預設 6.0
if "selected_magnitude" not in st.session_state:
    st.session_state.selected_magnitude = 6.0

mag = st.sidebar.number_input(
    "或手動輸入地震規模",
    value=st.session_state.selected_magnitude,
    step=0.1,
    key="manual_mag"
)
st.session_state.selected_magnitude = mag
...
if st.session_state.simulated:
    with st.spinner("正在計算受災程度與分配避難所..."):
        ...
        # SCENARIO COMPARISON (Improved with multiple colors and predefined magnitudes)
        st.subheader("🔄 快速場景比較")
        if st.checkbox("顯示多震度對比"):
            comp_mags = st.multiselect("選擇要比較的地震規模", [4.0, 5.0, 6.0, 7.0], default=[mag])
            
            if comp_mags:
                # Color mapping for different magnitudes
                color_map = {
                    4.0: "#1f77b4",  # Blue
                    5.0: "#ff7f0e",  # Orange
                    6.0: "#2ca02c",  # Green
                ...
                for comp_mag in comp_mags:
                    ...
                    comparison_data.append({
                        "地震規模": f"規模 {comp_mag}",
                        "預估難民數": refugees_count,
                        "顏色": color_map.get(comp_mag, "#999999")
                    })
                
                comp_df_display = pd.DataFrame(comparison_data)
                
                # Display comparison with colors and legend
                st.write("**各地震規模預估難民人數對比：**")
                col_chart_left, col_chart_right = st.columns([2, 1])
                
                with col_chart_left:
                    # Create custom bar chart
                    chart_data = comp_df_display.set_index("地震規模")["預估難民數"]
                    st.bar_chart(chart_data)
                
                with col_chart_right:
                    st.write("**詳細數據 & 顏色說明：**")
                    for idx, row in comp_df_display.iterrows():
                        st.write(f"<span style='color: {row['顏色']}; font-weight: bold;'>■ {row['地震規模']}</span><br/>{row['預估難民數']:,} 人", 
        ...
        with col_export1:
            ...
            st.download_button("📥 下載里別避難數據", csv, "refugee_data.csv", key="export_refugee")
        with col_export2:
            csv_flows = flows_df.to_csv(index=False, encoding="utf-8-sig")
            st.download_button("📥 下載分配方案", csv_flows, "allocation_plan.csv", key="export_allocation")
        with col_export3:
            csv_shelter = shelter_status.to_csv(index=False, encoding="utf-8-sig")
            st.download_button("📥 下載收容所狀態", csv_shelter, "shelter_status.csv", key="export_shelter")
